...
def _estimate_prompt_chars(events: list[dict]) -> int:
    """Rough estimate of how many chars build_enrichment_prompt would produce."""
    total = 0
    for ev in events:
        total += len(ev.get("command", "")) + 100  # metadata overhead
        total += len(ev.get("stdout", "") or "")
        total += len(ev.get("stderr", "") or "")
    return total


def _chunk_events_by_budget(events: list[dict], max_prompt_chars: int) -> list[list[dict]]:
    """Split events into chunks by time gaps or prompt size budget."""
    TIME_GAP_MS = 60_000
    chunks = []
    current = [events[0]]
    current_chars = _estimate_prompt_chars(current)

    for ev in events[1:]:
        prev_ts = current[-1]["timestamp"]
        gap = ev["timestamp"] - prev_ts
        ev_chars = _estimate_prompt_chars([ev])

        if gap > TIME_GAP_MS or current_chars + ev_chars > max_prompt_chars:
            chunks.append(current)
            current = [ev]
            current_chars = ev_chars
        else:
            current.append(ev)
            current_chars += ev_chars
...
def claim_pending_events_by_cwd(
    conn, worker_id: str, max_prompt_chars: int = 48000, stale_secs: int = 0
) -> list[list[dict]]:
    """Claim pending events grouped by cwd (project affinity). No fixed batch ceiling.

    Groups events by working directory, orders by timestamp within each group,
    and chunks by time gaps >60s or when estimated prompt size exceeds max_prompt_chars.

    stale_secs=0 processes ALL pending events including active sessions.
    """
    now_ms = int(time.time() * 1000)
    stale_lock_ms = now_ms - STALE_LOCK_TIMEOUT_MS
    stale_threshold_ms = now_ms - (stale_secs * 1000) if stale_secs > 0 else now_ms + 1

    # Get distinct cwds with pending events
    if stale_secs > 0:
        cursor = conn.execute(
            """
            SELECT e.cwd, COUNT(*) as cnt
            FROM enrichment_queue eq
            JOIN events e ON eq.event_id = e.id
            WHERE eq.status = 'pending'
               OR (eq.status = 'processing' AND COALESCE(eq.locked_at, 0) <= ?)
    ...
    for cwd, _ in cwd_groups:
        cursor = conn.execute(
            ...
            RETURNING event_id
            """,
            (now_ms, worker_id, now_ms, cwd, stale_lock_ms),
        )
        event_ids = [row[0] for row in cursor.fetchall()]
        conn.commit()
        ...
        chunks = _chunk_events_by_budget(events, max_prompt_chars)
        all_chunks.extend(chunks)
