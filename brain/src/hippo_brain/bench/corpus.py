...
def load_corpus(fixture_path: Path) -> Iterable[CorpusEntry]:
    """Stream CorpusEntry objects from a JSONL fixture, verifying per-row hashes."""
    with fixture_path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if stored is not None and entry.content_sha256 != stored:
                raise ValueError(
                    f"corpus entry {obj['event_id']!r} content hash mismatch "
                    f"(stored {stored} vs recomputed {entry.content_sha256})"
                )
            yield entry
...
def sample_from_hippo_db(
    ...
    *,
    ...
    # Stash bucket_index on the entry so write_corpus_jsonl can pick it up.
    for s in sampled:
        s.entry.__dict__["_bucket_index"] = s.bucket_index
...
def write_corpus_sqlite(
    ...
    *,
    ...
    `source_rows` carries the live sqlite3.Rows from sample_from_hippo_db;
...
def write_corpus_jsonl(
    ...
    *,
    ...
    with dest_jsonl.open("w", encoding="utf-8") as f:
        for e in entries:
            bidx = e.__dict__.get("_bucket_index", 0)
            obj = {
                "event_id": e.event_id,
                "source": e.source,
...
def _sample_with_rows(
    ...
    *,
    ...
    """Run sample_from_hippo_db and recover the underlying rows for sqlite write."""
...
def init_corpus(
    ...
    *,
    ...
    try:
        ...
        write_corpus_jsonl(entries, dest_jsonl, sampled_at_iso=sampled_at)
        ...
        if sorted(sqlite_event_ids) != jsonl_event_ids:
            raise AssertionError("corpus mismatch: sqlite event IDs differ from jsonl event IDs")
...
def verify_corpus(sqlite_path: Path, jsonl_path: Path, manifest_path: Path) -> tuple[bool, str]:
