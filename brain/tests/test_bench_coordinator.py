"""Tests for the coordinator's per-model lifecycle.

Focus: failure-recovery contract — when any step in run_one_model raises,
...
from hippo_brain.bench import coordinator
...
def _patch_lifecycle(
    ...
    *,
    ...
    monkeypatch.setattr(coordinator, "spawn_shadow_stack", spawn_mock)
    monkeypatch.setattr(coordinator, "wait_for_brain_ready", wait_mock)
    ...
    monkeypatch.setattr(coordinator, "_load_corpus_entries", MagicMock(return_value=[]))

    # Patch metrics sampler — we don't want a thread spinning.
    sampler_mock = MagicMock()
    sampler_mock.peak.return_value = {}
    monkeypatch.setattr(coordinator, "MetricsSampler", MagicMock(return_value=sampler_mock))
...
def test_wait_for_queue_drain_raises_on_missing_tables(tmp_path: Path) -> None:
    ...
    with pytest.raises(RuntimeError, match="no queue tables present"):
        coordinator._wait_for_queue_drain(bench_db, drain_timeout_sec=10.0, poll_interval_sec=0.1)
...
def test_wait_for_queue_drain_returns_drained_when_tables_empty(tmp_path: Path) -> None:
    ...
    timeout_hit = coordinator._wait_for_queue_drain(
...
def test_smoke_run_one_model_full_lifecycle(
    monkeypatch: pytest.MonkeyPatch, fake_corpus: Path
) -> None:
...
def test_load_corpus_failure_captured_as_structured_error(
    ...
    SC pass naturally skipped) rather than crashing run_one_model.
    """
    mocks = _patch_lifecycle(monkeypatch)
    monkeypatch.setattr(
        coordinator,
