"""Tests for hippo_brain.bench.corpus — time-bucketed sample, shadow SQLite,
...
def test_time_bucket_coverage(tmp_db):
    ...
    buckets_hit = {e.__dict__.get("_bucket_index", -1) for e in entries}
...
# ── 6. verify_corpus passes on fresh corpus ──────────────────────────────


def test_verify_corpus_passes(seeded_db, tmp_path):
    _, db_path = seeded_db
    dest_sqlite = tmp_path / "corpus-v2.sqlite"
    dest_jsonl = tmp_path / "corpus-v2.jsonl"
    manifest = tmp_path / "corpus-v2.manifest.json"
    ...
    ok, detail = verify_corpus(dest_sqlite, dest_jsonl, manifest)
    assert ok, detail
    assert detail == "ok"


# ── 7. verify_corpus detects sqlite tampering ────────────────────────────


def test_verify_corpus_fails_on_tamper(seeded_db, tmp_path):
    _, db_path = seeded_db
    dest_sqlite = tmp_path / "corpus-v2.sqlite"
    dest_jsonl = tmp_path / "corpus-v2.jsonl"
    manifest = tmp_path / "corpus-v2.manifest.json"
