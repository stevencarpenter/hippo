...
fn upsert_writes_agentic_session_and_enqueues() {
    ...
    let (cnt, src): (i64, String) = conn
        .query_row(
            "SELECT COUNT(*), MAX(source_file) FROM agentic_sessions
             WHERE session_id = 'codex-1' AND harness = 'codex'",
            [],
    ...
    let queued: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM agentic_enrichment_queue q
             ...
             WHERE s.session_id = 'codex-1' AND s.harness = 'codex' AND q.status = 'pending'",
            [],
            |r| r.get(0),
        )
        .unwrap();
    ...
    let queued2: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM agentic_enrichment_queue q
             JOIN agentic_sessions s ON s.id = q.session_id
             WHERE s.session_id = 'codex-1' AND s.harness = 'codex'",
            [],
            |r| r.get(0),
        )
        .unwrap();
...
fn upsert_refreshes_cwd_and_project_dir_on_conflict() {
    ...
    let (cwd, project_dir): (String, String) = conn
        .query_row(
            "SELECT cwd, project_dir FROM agentic_sessions
             WHERE session_id = 'cwd-test' AND harness = 'codex'",
            [],
            |r| Ok((r.get(0)?, r.get(1)?)),
        )
        .unwrap();
