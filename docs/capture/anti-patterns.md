...
**TL;DR:** Twelve concrete failure modes observed in hippo's capture layer, each with a forbidden behavior, the reason it is forbidden, and the required alternative. Any of these patterns in a PR is a review blocker.

For the system reference these rules guard, see [`architecture.md`](architecture.md). For per-source detail, see [`sources.md`](sources.md). Historical design records are archived under [`docs/archive/capture-reliability-overhaul/`](../archive/capture-reliability-overhaul/).
...
**The right way.** Watchdog and doctor gate silence alarms on corroborating signals drawn from data hippo already captures, not new OS-level process/idle-time checks: source process confirmed running (`pgrep -x firefox` for browser), backing session files changed recently (Codex/Cursor idle probes; opencode uses its `session.time_updated` DB column instead, since the DB file's own mtime is always recent regardless of session activity), and for shell, the recency of a real (`probe_tag IS NULL`) shell event (`shell_health::shell_real_activity_recent`). The synthetic probe alone cannot serve as that signal, since it lands on its own 5-minute cadence regardless of user presence. Thresholds per-source, configurable, conservative defaults.
...
**Where seen:** `crates/hippo-daemon/src/claude_session.rs::insert_segments` until 2026-04-27 (see T-A.3 and [`docs/archive/capture-reliability-overhaul/11-watcher-data-loss-fix.md`](../archive/capture-reliability-overhaul/11-watcher-data-loss-fix.md) Phase 1).
