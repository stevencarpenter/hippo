...
plus the two hotfixes #54/#55) and every invariant from architecture.md must
...
| F-33 | Cursor probe fails when `~/.cursor/projects` is absent or contains no settled transcripts | probe trivial-pass path | rust unit | `crates/hippo-daemon/src/probe_agentic.rs::tests::cursor_probe_trivial_pass_when_no_transcripts` | new (cursor-ingestion PR) | I-15 |
| F-35 | Codex probe fails when no settled `rollout-*.jsonl` in eligibility window | probe trivial-pass path | rust unit | `crates/hippo-daemon/src/probe_agentic.rs::tests::codex_probe_trivial_pass_when_no_rollouts` | new (SNUG-117) | I-13 |
| F-36 | Opencode probe fails when no sessions in eligibility window | probe trivial-pass path | rust unit | `crates/hippo-daemon/src/probe_agentic.rs::tests::opencode_probe_trivial_pass_when_no_sessions` | new (SNUG-117) | I-11 |
| F-37 | Claude-session ingest retries transient SQLITE_BUSY without error-level log or file cooldown | watcher write contention | rust unit | `crates/hippo-daemon/src/claude_session.rs::tests::test_ingest_session_file_retries_transient_busy`, `crates/hippo-daemon/src/watch_claude_sessions.rs::tests::process_file_no_cooldown_when_insert_succeeds_under_contention` | new (SNUG-110) | — |
...
architecture.md that is not in the table above, that is a gap — open an
issue and add a row.
