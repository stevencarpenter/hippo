...
| **I-16** Agentic node dedup | No single `agentic_sessions` segment may carry more than one `observation` knowledge node with identical `(content, embed_text, node_type)`. A non-zero count means agentic enrichment is appending duplicate nodes instead of replacing them (the historical KB-duplication class — see AP-13). Scoped to `observation` (the class `replace_prior_agentic_nodes` guards); the still-unguarded workflow `change_outcome` co-links are a tracked follow-up. | 0 dup-groups (configurable via `[watchdog] dup_node_alarm_threshold`) | `knowledge_node_agentic_sessions` absent — fresh install. | Watchdog alarm. Remediate with `brain/scripts/dedup-knowledge-nodes.py`. |
