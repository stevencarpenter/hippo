...
| **I-4** Browser round-trip | If extension heartbeat is recent (< 2 min), browser events (user visits or synthetic probe) must land within one probe cadence plus jitter grace. | 7 min | Extension heartbeat absent or stale; Firefox not running (doctor suppresses). | Watchdog alarm + doctor `[!!] browser events` with connectivity state. |
