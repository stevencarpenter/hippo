...
| **I-8** Probe freshness | For each source with `probe_last_run_ts IS NOT NULL`: `probe_ok = 1` OR `probe_last_run_ts > now − 15 min`. | 15 min | `shell` only: probe stale (not outright failing) AND no real shell event within 10 minutes either. Most likely the machine was asleep, not that shell-probe delivery broke. Never suppresses an outright probe failure (`probe_ok = 0`). | Watchdog alarm + doctor `[!!] <source> probe`. |
