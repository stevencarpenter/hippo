...
| 2 | **Claude tool events** | Derived during Claude-session ingest. `claude_session.rs::build_envelope` → same `flush_events` path; `storage.rs` derives `source_kind='claude-tool'` when `tool_name.is_some()` | `events` (`source_kind='claude-tool'`, `tool_name` set) | I-3 | Indirect (rides on claude-session probe). Freshness covered by `check_source_staleness` via `source_health`; excluded from `source_freshness_probes` because probe events inflate raw row counts and mask real capture health. | healthy |
| 3 | **Claude session segments** (FS watcher) | `com.hippo.claude-session-watcher` LaunchAgent → FSEvents on `~/.claude/projects/**/*.jsonl` → `watch_claude_sessions.rs::process_file` → `claude_session.rs::ingest_session_file` → `insert_segments` | `agentic_sessions`, `agentic_enrichment_queue` (capture-health key `agentic-session-claude`) | I-2 | Yes (synthetic JSONL append) | healthy |
| 4 | **Claude subagent sessions** (`agent-*.jsonl`) | Same FS-watcher path; `SessionFile::from_path` detects `<project>/<parent-uuid>/subagents/<id>.jsonl` and sets `is_subagent=true` | `agentic_sessions` with `is_subagent=1`, `parent_session_id` | I-2 | (rides on parent) | healthy |
...
| 7 | **Xcode ClaudeAgentConfig sessions** | `com.hippo.xcode-claude-ingest` LaunchAgent → `scripts/hippo-ingest-claude.py` → Python `insert_segment` (same `agentic_sessions` schema) | `agentic_sessions`, `agentic_enrichment_queue` | I-2 (shared) | Indirect | healthy |
| 8 | **Codex sessions** | `com.hippo.codex-session` LaunchAgent (scheduled poller) → `hippo codex-poll` → `codex_session::poll_tick` (Rust) walks `~/.codex/sessions` (+ archived + Xcode `CodingAssistant/codex/sessions` as secondary) for `rollout-*.jsonl`, upserts segmented rows into `agentic_sessions` via `(session_id, harness, segment_index)` ON CONFLICT; capture-health key `agentic-session-codex` | `agentic_sessions` (shared), `agentic_enrichment_queue` | I-2 (shared) | Yes (assertion-only: settled `rollout-*.jsonl` → matching `agentic_sessions` row) | healthy — Rust poller, same source-audit coverage as other `agentic_sessions` sources |
| 9 | **Cursor Agent sessions** | `com.hippo.cursor-session` LaunchAgent (scheduled poller) → `hippo cursor-poll` → `cursor_session::poll_tick` (Rust) walks `~/.cursor/projects/**/agent-transcripts/**/*.jsonl`, parses char-bounded segments stamped from file mtime, upserts segmented rows into `agentic_sessions` via `(session_id, harness, segment_index)` ON CONFLICT; capture-health key `agentic-session-cursor` | `agentic_sessions` (shared), `agentic_enrichment_queue` | I-15 | Assertion-only (settled transcripts present → matching `agentic_sessions` row) | healthy — Rust poller, same source-audit coverage as other `agentic_sessions` sources |
| 10 | **Probe events** | `com.hippo.probe` LaunchAgent → `crates/hippo-daemon/src/probe.rs` → per-source synthetic-event path | `events` / `browser_events` / `agentic_sessions` with `probe_tag IS NOT NULL` | I-8 | (drives the others' probes) | healthy |
...
| 14 | **Pi agent sessions** | `com.hippo.pi-session` LaunchAgent (scheduled poller) → `hippo pi-poll` → `pi_session::poll_tick` (Rust) walks `~/.pi/agent/sessions/<slug>/` for main-session and subagent `session.jsonl` transcripts plus reviewer transcripts, upserts segmented rows into `agentic_sessions` (harness `'pi'`) via `(session_id, harness, segment_index)` ON CONFLICT; capture-health key `agentic-session-pi` | `agentic_sessions` (shared), `agentic_enrichment_queue` | I-2 (shared) | Yes (assertion-only: settled transcripts → matching `agentic_sessions` row, `hippo probe agentic-session-pi`) | healthy — Rust poller, same source-audit coverage as other `agentic_sessions` sources |
...
Two distinct sources write to `agentic_sessions`:

1. The **FS watcher** (`com.hippo.claude-session-watcher`, KeepAlive=true) is the canonical real-time path. FSEvents on `~/.claude/projects/**/*.jsonl` triggers `extract_segments`; segments are upserted via `(session_id, harness, segment_index)` ON CONFLICT. The legacy per-session tmux tailer was deleted in T-8 (PR #89); the `SessionStart` hook is now a no-op debug log.
...
Two LaunchAgents write into the shared `agentic_sessions` table. The ClaudeAgentConfig path (`com.hippo.xcode-claude-ingest`) still uses the Python script `scripts/hippo-ingest-claude.py`. The Codex rollout path (`com.hippo.codex-session`) is a Rust poller: `hippo codex-poll` runs `codex_session::poll_tick`, which walks `~/.codex/sessions` (+ archived + the Xcode CodingAssistant directory) for `rollout-*.jsonl` files, parses their distinct JSONL envelope (`session_meta`, `response_item/function_call`) natively in Rust, and upserts segments into `agentic_sessions`. Both paths share the `agentic_enrichment_queue` enrichment path.

### Cursor Agent sessions

The Cursor poller (`com.hippo.cursor-session`) is a Rust poller: `hippo cursor-poll` runs `cursor_session::poll_tick`, which walks `~/.cursor/projects` recursively for `agent-transcripts/**/*.jsonl` files (main sessions and subagents). Cursor transcripts carry no per-line timestamps; segments are bounded by accumulated character count and time-stamped from the file mtime. Subagents are ingested as their own `agentic_sessions` rows with `is_subagent=1` and `parent_session_id` set. The poller shares the `agentic_enrichment_queue` enrichment path with Claude Code, Codex, and other `agentic_sessions` sources. Capture-health key is `agentic-session-cursor`; watchdog invariant I-15 gates on `consecutive_failures > 3`.
...
Opencode writes `agentic_sessions` with `harness='opencode'`, sharing the table
and enrichment queue with Claude Code, Codex, Cursor, and Pi.

Each poll compares the source session's `time_updated` with its stored `end_time`
in `agentic_sessions`. New or changed sessions are upserted; a failed transaction
leaves the previous watermark intact for retry. Unchanged sessions are skipped.
Opencode does not use `agentic_cursor`, which stores per-file poller cursors.
...
The brain side mirrors `agentic_sessions.py`: `claim_pending_opencode_segments` flips queue rows to `processing`, the LLM call produces a `knowledge_nodes` row, `write_opencode_knowledge_node` links via `knowledge_node_agentic_sessions` and closes out the queue entry. Eligibility filter (in `enrichment.py::is_enrichment_eligible`) skips sessions with `<3` messages and no diffs/commits.

No production probe yet — `hippo probe --source agentic-session-opencode` and `agentic-session-codex` run assertion-only canaries (see rows 8 and 12). The doctor freshness check uses the opencode DB's own mtime as a suppression signal so an idle day in opencode doesn't fail the run.
