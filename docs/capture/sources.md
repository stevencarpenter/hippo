...
| 14 | **Pi agent sessions** | `com.hippo.pi-session` LaunchAgent (scheduled poller) → `hippo pi-poll` → `pi_session::poll_tick` (Rust) walks `~/.pi/agent/sessions/<slug>/` for main-session and subagent `session.jsonl` transcripts plus reviewer transcripts, upserts segmented rows into `agentic_sessions` (harness `'pi'`) via `(session_id, segment_index)` ON CONFLICT; capture-health key `agentic-session-pi` | `agentic_sessions` (shared), `agentic_enrichment_queue` | I-2 (shared) | Yes (assertion-only: settled transcripts → matching `agentic_sessions` row, `hippo probe agentic-session-pi`) | healthy — Rust poller, same source-audit coverage as other `agentic_sessions` sources |
