...
- The probe's `probe_ok` definition is source-specific (e.g. shell: the last synthetic probe round-trip through the real capture path succeeded, an unconditional canary independent of user presence; browser: the last synthetic `hippo probe --source browser` round-trip succeeded, likewise independent of whether Firefox is currently running (Firefox process state is a separate I-4 doctor/watchdog suppression signal, not `probe_ok` itself); claude-session: at least one fresh-mtime JSONL in `~/.claude/projects`). Document yours. If your source needs an idle-vs-broken distinction the way shell does (see `shell_health::shell_real_activity_recent`), do not rely on `probe_ok` alone for it: the probe cadence itself can mask genuine idleness. Derive idleness from a signal the probe cannot touch, such as the recency of a real, non-probe-tagged event.
