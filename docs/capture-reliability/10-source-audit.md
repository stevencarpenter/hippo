...
| 3 | Claude session segments (batch import) | `claude_session.rs:990` (`ingest_batch`) → `write_session_segments` (L955) → `insert_segments` (L845) direct SQLite | `claude_sessions`, `claude_enrichment_queue` | `tests/source_audit.rs::claude_session_batch` | healthy | Fixed in #59 — previously only events flowed. |
| 4 | Claude session segments (FS watcher) | `crates/hippo-daemon/src/watch_claude_sessions.rs:118` (`process_file`) → `claude_session::ingest_session_file` → `insert_segments` direct SQLite | `claude_sessions`, `claude_enrichment_queue` | `crates/hippo-daemon/tests/claude_session_watcher_integration.rs` | healthy | The `com.hippo.claude-session-watcher` LaunchAgent (KeepAlive=true) subscribes to FSEvents on `~/.claude/projects/**/*.jsonl` and re-runs `extract_segments` whenever a file grows. Per-file resume state lives in `claude_session_offsets`. Replaced the per-session tmux tailer in T-8 (PR #89). |
...
| 7 | Claude subagent sessions (`agent-*.jsonl`) | same `ingest_batch` / FS watcher path; `SessionFile::from_path` (L396) detects `<project>/<parent-uuid>/subagents/<id>.jsonl` and sets `is_subagent=true` | `claude_sessions` with `is_subagent=1`, `parent_session_id=<parent uuid>` | `tests/source_audit.rs::claude_subagent` | healthy | Subagent segments are enqueued for enrichment like main segments. |
...
2. ~~**Tailer watcher (FS-native)**~~ — Resolved by T-5 (PR #86) and T-8
   (PR #89). The FS watcher (`com.hippo.claude-session-watcher`,
   KeepAlive=true) is the sole production ingester. The legacy per-session
   tmux tailer was deleted in T-8; the `SessionStart` hook is now a no-op
   debug log. Live sessions are picked up by FSEvents within ~1 s.
