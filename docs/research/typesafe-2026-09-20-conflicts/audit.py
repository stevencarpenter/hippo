...
def audit(root):
    ...
    for name, expected in manifest["implementation_hashes"].items():
        require(hashlib.sha256((root / "source" / name).read_bytes()).hexdigest() == expected,
    ...
    for arm in manifest["arms"]:
        ...
        for line in path.read_text().splitlines():
            ...
            require(row["arm"] == arm and key in planned and key not in records, "invalid row identity")
            ...
            if "request" in row:
                ...
                state = request["state"] if "state" in request else json.loads(request["messages"][1]["content"])
                require(state == planned[key], "different inference input or leaked annotations")
                if row["status"] == "ok":
                    response = row["response"]
                    verdict = (response["answers"]["verdict"]["choice"] if "state" in request
                               else json.loads(response["choices"][0]["message"]["content"])["verdict"])
                    require(verdict == row["output"]["verdict"], "recorded verdict differs from response")
            records[key] = row
    groups, cases, paired = [], [], []
    for task in sorted({case["task"] for case in inputs}):
        ids = [case["id"] for case in inputs if case["task"] == task]
        for arm in manifest["arms"]:
            rows = [row for key, row in records.items() if key[0] == task and key[3] == arm]
            latency = sorted(row["elapsed_ms"] for row in rows)
            confusions = {"true_conflicts": 0, "true_compatible": 0, "false_conflicts": 0,
                          "missed_conflicts": 0, "error": 0, "abstain": 0, "missing": 0}
            ...
            for case_id in ids:
                ...
                for repeat in range(manifest["repeats"]):
                    ...
                    confidence = (row or {}).get("output", {}).get("judgments", {}).get("verdict", {}).get("confidence")
                    confidences.append(confidence)
                    if status != "ok":
                        confusions[status] += 1
                    elif expected == verdict:
                        confusions["true_conflicts" if verdict == "conflict" else "true_compatible"] += 1
                    else:
                        confusions["false_conflicts" if verdict == "conflict" else "missed_conflicts"] += 1
                consistent_correct += all(verdict == expected for verdict in verdicts)
                unstable += len(set(verdicts)) > 1
                cases.append({"task": task, "case_id": case_id, "arm": arm, "expected": expected,
                              "verdicts": verdicts, "confidence": confidences})
            groups.append({"task": task, "arm": arm, "unique_cases": len(ids),
                           "correct_in_both_orders": consistent_correct, "changed_between_orders": unstable,
                           "correct_decisions": confusions["true_conflicts"] + confusions["true_compatible"],
                           "planned_decisions": len(ids) * manifest["repeats"],
                           **confusions, "network_calls": sum(row["network_calls"] for row in rows),
                           "p50_ms": statistics.median(latency) if latency else None,
                           "p95_ms": latency[math.ceil(len(latency) * .95) - 1] if latency else None})
        for arm, baseline in (("jev", "current"), ("llm", "current"), ("jev", "llm"),
                              ("rules_jev", "jev"), ("rules_llm", "llm")):
            ...
            for case_id in ids:
                ...
                for candidate in (arm, baseline):
                    group = [records.get((task, case_id, repeat, candidate)) for repeat in range(manifest["repeats"])]
                    correct[candidate] = all(row and row["status"] == "ok" and row["output"]["verdict"] == expected for row in group)
                target = ties if correct[arm] == correct[baseline] else wins if correct[arm] else losses
    ...
    for rows in repeated:
        if len({row["output"]["verdict"] for row in rows}) > 1:
            variations.append({"request_hash": rows[0]["request_hash"], "calls": [
                {"case_id": row["case_id"], "arm": row["arm"], "repeat": row["repeat"],
                 "verdict": row["output"]["verdict"]} for row in rows
            ]})
    summary_path = root / "summary.json"
    if summary_path.exists():
        saved = json.loads(summary_path.read_text())
        require(saved["observed_records"] == len(records), "saved record count mismatch")
        require(saved["expected_records"] == len(planned), "saved planned count mismatch")
        for group in groups:
            original = next(g for g in saved["groups"] if (g["task"], g["arm"]) == (group["task"], group["arm"]))
            for current_key, saved_key in (("correct_decisions", "correct"), ("error", "errors"),
                                           ("abstain", "abstentions"), ("missing", "missing"),
                                           ("false_conflicts", "false_conflicts"),
                                           ("missed_conflicts", "missed_conflicts"),
                                           ("network_calls", "network_calls")):
                require(group[current_key] == original[saved_key], "saved metric mismatch: " + saved_key)
