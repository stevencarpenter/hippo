...
| Launch-time `uv run` re-synced the verified non-editable brain venv to an editable install | `--no-sync` on the brain and Xcode-ingest LaunchAgents and the Rust auto-memory callers | Reproduced with real uv on the brain package; plist regression test. |
| Data-directory check aborted every CLI command, including the shell hook | Warn only for `send-event`, `native-messaging-host`, `doctor`, and `config`; other commands refuse; doctor reports the shared check | Symlink, unwritable-target, and home-directory scenarios run against the built binary. |
| Doctor reported a healthy Jev endpoint when no client existed | Report the endpoint as not configured | Unit test. |
| Installer tests had no CI job; failed extraction left staging directories | Linux installer workflow; staging removed on extraction failure | New scenario fails against the prior installer. |
...
On September 25, 2026, a separate review of `888a413` found the four defects added to the corrective-changes table. It also found the duplicated `fix: fix:` commit subject, which came from the commit-message template. The template no longer adds a prefix a summary already has. After those fixes, the full local suite on the working tree above `888a413` passed **1,725 Python tests**, with **1 expected failure** and **85% coverage**. Rust passed **1,156 test executions** with no failures. All five installer scenarios, lint, formatting, release-feature Clippy, and `git diff --check` passed. The three installer test scripts also passed in an Ubuntu 24.04 container. This run is local validation, not release-SHA CI. It does not change any release gate.
