...
| **v19** | Add read-only Claude Code auto-memory ingestion. | Adds document identity, immutable revisions, deterministic chunks, enrichment queue, knowledge-node links, categories, and memory links. Seeds `source_health.source = 'claude-auto-memory'`. The migration is additive and idempotent; no existing rows are rewritten. | The brain and daemon require v19. Disable `[auto_memory]` to stop ingestion without removing stored projections. |
