...
The normal Rust migration owns schema **25**. Install matching Rust and Python components through the normal deployment workflow before enabling classification. This reference does not perform that deployment. Python can still read schema 24 for ordinary queries and enrichment; missing classification schema disables classification with a diagnostic. Do not apply experimental SQL directly to the production database. Classification CLI commands do not migrate it.
