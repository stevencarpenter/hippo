...
| `hippo doctor` says inference server isn't reachable | omlx/LM Studio isn't running, or it's serving on a non-default port. | Start the server. Check the API base URL in `~/.config/hippo/config.toml` (`[inference].base_url`; legacy `[lmstudio].base_url` still accepted). |
