...
Ingestion is handled by `cursor_session::poll_tick` in `crates/hippo-daemon/src/cursor_session.rs`, invoked by the `hippo cursor-poll` CLI command. It runs under launchd (`com.hippo.cursor-session`), `StartInterval`-driven on the `[cursor] poll_interval_secs` cadence (default 60 s). Each tick walks the `[cursor] session_roots` directories (default: `~/.cursor/projects`) for `agent-transcripts/**/*.jsonl` files. It skips in-flight files (modified within `min_idle_secs`, default 60 s, to avoid partial reads) and skips unchanged files via an inode-keyed cursor in the `agentic_cursor` table (`source_key = cursor-{inode}`).
